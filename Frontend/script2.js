...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    // Fetch reports from backend
    const response = await fetch('http://localhost:5000/api/get-reports'); // Adjust URL if deployed
    const result = await response.json();
  ...
  function showModal(report) {
    ...
    `;
    console.log("Image URL being used:", report.evidencePhoto);
    console.log("Modal content:", modalContent); // Debug: Check the modal HTML content
    modalBody.innerHTML = modalContent;
    modal.style.display = 'block'; // Show the modal
  }
